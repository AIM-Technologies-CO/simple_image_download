...
def generate_urls(search):
    ...
    print([quote(word) for word in search])
...
class Downloader:
    ...
    def search_urls(self, keywords, limit=1, verbose=False, cache=True, timer=None):
        ...
        while i < count:
            ...
            raw_html = self._download_page(
                url
            )  # Download the entire page from the google Picture search
    ...
    def download(
        self, keywords=None, limit=1, verbose=False, cache=True, download_cache=False, timer=None
    ):
        if not download_cache:
            ...
            if not content:
                print(
                    "Downloader has not URLs saved in Memory yet, run Downloader.search_urls to find pics first"
                )
        for name, (path, url) in content.items():
            new_dir_name = path.split("/")[-1].replace("_", "")
            new_name = name.replace(" ", "")
            new_path = os.path.join("".join(path.split("/")[:-1]), new_dir_name)
            print(new_path)
            print(os.path.join(new_path, new_name))
            with open(os.path.join(new_path, new_name), "wb") as file:
                file.write(url.content)
            if verbose:
                print(f"File Name={new_name}, Downloaded from {url.url}")
            else:
                pass
